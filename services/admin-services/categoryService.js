...
const getCategories = async (perPage,page) => {
   let categories = await Category.find({}).sort({createdAt : -1}).skip(perPage * page - perPage).limit(perPage)
   return categories
}
...
const doesCategoryExist = async (categoryName) => {
    let category = await Category.findOne({ categoryName : categoryName.toUpperCase() })
    return category
}
