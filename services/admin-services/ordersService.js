...
const getOrder = async (id) => {
   let order = await Order.findOne({_id : id}).populate("userId").populate("paymentId")
   return order
}
        ...
        if(order.items[i].isCancelled){
            tableRow += `<tr>
                            <td style="padding: 10px; border-bottom: 1px solid #eee;">
                                <p style="font-weight: bold; margin: 0;">${ order.items[i].productName }(Cancelled)</p>
    ...
    return ` <!DOCTYPE html>
            ...
            <div style="display: flex; justify-content: space-between; margin-bottom: 30px;">
                ...
                <div style="text-align: right; ">
                    ...
                    <p style="margin: 5px 0 0; color:#404040;">Date:  ${new Date(order.invoiceCreatedAt).toLocaleDateString("en-IN", {
                        day: "numeric",
                        month: "short",
                        year: "numeric"
                    })}</p>
            ...
            <div style="display: flex; justify-content: space-between; margin-bottom: 30px;">
                <div style="width: 48%;">
                    ...
                    <p style="margin: 5px 0; font-weight: bold;">${order.address.firstName} ${order.address.lastName}</p>
                    <p style="margin: 5px 0;">${order.address.address}</p>
                    <p style="margin: 5px 0;">${order.address.state} ${(order.address.pincode !== "Not Applicable") ? order.address.pincode : ""}</p>
