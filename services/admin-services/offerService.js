  ...
  if (newOffer.applicableOn === "product") {
    ...
    if (product.variants[newOffer.productVariant].productOfferId !== null) {
      await Offer.findOneAndDelete({
        _id: product.variants[newOffer.productVariant].productOfferId,
      });
      product.variants[newOffer.productVariant].productOfferId = newOffer._id;
      await Cart.updateMany(
        { productId: product._id, variant: newOffer.productVariant },
...
const updateOffer = async (
  ...
  if (offerType === "product") {
    ...
    await product.save();
    await Cart.updateMany(
      { productId: selectedTarget, variant: selectedTargetVariant },
      { productOfferId: id }
    );
  } else {
    await Product.updateMany(
