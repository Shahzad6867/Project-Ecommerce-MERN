    ...
    if (session.metadata?.for === "Wallet") {
      if (event.type === "checkout.session.completed") {
        const payment = await Payment.findOne({
          _id: new mongoose.Types.ObjectId(session.metadata.payment),
        });
      ...
      if (event.type === "checkout.session.completed") {
        ...
        await Cart.deleteMany({
          userId: new mongoose.Types.ObjectId(session.metadata.user),
        });
        payment.status = "Paid Successfully";
        payment.amountPaid = payment.amountToBePaid;
        payment.amountToBePaid = 0;
    ...
    if (
      ...
      for (let i = 0; i < order.items.length; i++) {
        if (
          ...
          order.items[i].refundOnCancelled.refundId === event.data.object.id &&
