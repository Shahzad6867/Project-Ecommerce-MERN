...
const getProductDetail = async (req, res) => {
  try {
    ...
    const product = await homeService.getProduct(id);
    if (product.isDeleted) {
      req.session.message = "Product Unavailable";
      return res.redirect("/");
    }
    const relatedProduct = await homeService.getRelatedProducts(
...
const getShop = async (req, res) => {
  try {
    ...
    if (category) {
      query.categoryId = {
        $in: (Array.isArray(category) ? category : [category]).map(
    ...
    if (brand) {
      query.brandId = {
        $in: (Array.isArray(brand) ? brand : [brand]).map(
          (id) => new mongoose.Types.ObjectId(id)
        ),
      };
