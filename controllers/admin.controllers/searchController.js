...
const searchProducts = async (req, res) => {
  try {
    ...
    const productsFullList = await Product.aggregate().project({
      productName: 1,
      _id: 0,
    });
    ...
    let message = null;
    return res.render("admin-view/admin.searched-product-managment.ejs", {
      products,
      search,
      productsFullList,
      message,
...
const searchOrders = async (req, res) => {
  try {
    ...
    if (!orders) {
      req.session.message =
        "There is no existing Order based on the given Order ID";
      return res.redirect("/admin/orders");
    }
    let message = null;
    return res.render("admin-view/admin.searched-order-management.ejs", {
      orders,
      search,
      ordersFullList,
      message,
