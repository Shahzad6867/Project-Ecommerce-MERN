...
const getSalesReport = async (req, res) => {
  try {
    ...
    let totalOrdersCount = await reportsService.computeTotalOrdersCount(
      fromDate,
      toDate
    );
    ...
    let totalProductDiscount = await reportsService.computeTotalProductDiscount(
      fromDate,
      toDate
    );
    let totalCouponDiscount = await reportsService.computeTotalCouponDiscount(
      fromDate,
      toDate
    );
    let totalRefunds = await reportsService.computeTotalRefunds(
      fromDate,
      toDate
    );
    let totalTax = await reportsService.computeTotalTax(fromDate, toDate);
    let totalShipping = await reportsService.computeTotalShipping(
      fromDate,
      toDate
    );
    let totalOrderValue = await reportsService.computeTotalOrderValue(
      fromDate,
      toDate
    );
    let netSales = await reportsService.computeNetSales(fromDate, toDate);
    let salePerItem = await reportsService.computeSalePerItem(
      fromDate,
      toDate,
      skipForProduct,
      perPage
    );
    let orderBasedSales = await reportsService.computeOrderBasedSales(
      fromDate,
      toDate,
      skipForOrder,
      perPage
...
const getSalesReportIntoExcel = async (req, res) => {
  try {
    ...
    let generatedOn = today.toLocaleDateString("en-IN", {
      day: "numeric",
      month: "short",
      year: "numeric",
    });
    ...
    let salePerItem = await reportsService.computeSalePerItemForPdfAndExcel(
      fromDate,
      toDate
    );
    let orderBasedSales =
      await reportsService.computeOrderBasedSalesForPdfAndExcel(
        fromDate,
        toDate
      );
    ...
    const productWiseSalesReport = workbook.addWorksheet(
    ...
    const orderWiseSalesReport = workbook.addWorksheet(
...
const getSalesReportIntoPdf = async (req, res) => {
  try {
    ...
    let generatedOn = today.toLocaleDateString("en-US", {
      day: "numeric",
      month: "short",
      year: "numeric",
      timeZone: req.cookies.tz,
    ...
    await doc.table({
      ...
      rows: orderBasedSales.map((order, i) => [
        i + 1,
        `${order._id}`,
        order.orderedAt.toLocaleDateString("en-IN", {
          day: "numeric",
          month: "short",
          year: "numeric",
        }),
