  ...
  fetch(`/cart?productId=${productId}&variant=${variant}&quantity=1`, {
    ...
    .then((data) => {
      if (data.message === "Product has been added to Cart") {
        ...
        document
          .getElementById(`incDecQtyDiv${productId}${variant}`)
        ...
        subTotal.innerText =
          Math.round((Number(subTotal.innerText) + price) * 100) / 100;
        feather.replace();
        iziToast.info({
          title: "Cart",
    ...
    fetch(
      ...
      .then((data) => {
        if (data.message === "Quantity has been updated in Cart") {
          ...
          document
            .getElementById(`incDecQtyDiv${productId}${variantIndex}`)
...
function deleteItemFromCart(cartId, productId, variantIndex) {
  let quantity = document.getElementById(`quantity${productId}${variantIndex}`);
  let price =
    document.getElementById("isInCartPage")?.value !== "Yes"
      ? Number(
          document.getElementById(`price${productId}${variantIndex}`).innerText
        )
      : Number(
  ...
  fetch(`/cart/delete-cart-item?cartItemId=${cartId}&productId=${productId}`, {
    ...
    .then((data) => {
      if (document.getElementById("isInCartPage")?.value !== "Yes") {
        ...
        subTotal.innerText =
          Math.round(
            (Number(subTotal.innerText) - Number(quantity.value) * price) * 100
          ) / 100;
        iziToast.info({
          title: "Cart",
          message: data.message,
...
function getUserToLogin() {
  iziToast.info({
    title: "Info",
    message:
      "Almost there! Please log in to add items to your Cart or Wishlist",
    position: "topCenter",
  });
}
