feather.replace();
        ...
        function updateItemStatus(input,index) {
            if(input.value === "Return Request Declined"){
                document.getElementById(`declineItemReasonDiv${index}`).classList.remove("hidden") 
                document.getElementById(`declineItemReason${index}`).disabled = false
            }
            if(input.value === "Return Request Approved" || input.value === ""){
                document.getElementById(`declineItemReasonDiv${index}`).classList.add("hidden") 
        ...
        if(statusToBe !== "" && statusToBe !== "Return Request Declined"){
            ...
            fetch(`/admin/orders/${orderId}/update-status?status=${statusToBe}&item=${itemIndex}`,{method : "PATCH"})
            .then(res => window.location.href = `/admin/orders/${orderId}`)
        }else{
            let declineReason = document.getElementById(`declineItemReason${itemIndex}`).value
            ...
            btn.disabled = true
            document.getElementById("updationOverlay").classList.remove("hidden")
            fetch(`/admin/orders/${orderId}/update-status?status=${statusToBe}&item=${itemIndex}`,{
                method : "PATCH",
                headers : {
